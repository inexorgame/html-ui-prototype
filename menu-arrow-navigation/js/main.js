(function (document, undefined) {
	 ...
	 * @param direction: The direction where the focus should go. Down = 0, Left = 1, Up = 2, Right = 3
	 * @return jquery-object
	 */
	function focusOn(direction)
	{
		// Gets the central coords of an element
		var getCenterCoords = function (f) {
			if(f.length == 0) 
			{
				f = $('*[tabindex=1]').first();
				if(f.length == 0) return { x: $(document).width()/2, y: Infinity };
			}
			return { x : f.offset().left + f.width() / 2, y : f.offset().top + f.height() / 2 };
		}

		var focusedPoint = getCenterCoords($('*:focus'));
		var possibilities = $('*[tabindex]:not(:focus)');
